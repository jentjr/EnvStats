...
function (n, n.median = 1, k = 1, m = 2, r = 1, rule = "k.of.m",
    lpl.rank = ifelse(pi.type == "upper", 0, 1), n.plus.one.minus.upl.rank = ifelse(pi.type ==
        "lower", 0, 1), pi.type = "two-sided", integrate.args.list = NULL)
{
    ...
    if (pi.type %in% c("two-sided", "lower") & any(lpl.rank < 1))
        stop(paste("When pi.type='two-sided' or pi.type='lower', ",
                   "all values of 'lpl.rank' must be positive integers"))
    ...
    if (pi.type %in% c("two-sided", "upper") & any(n.plus.one.minus.upl.rank <
        1))
        stop(paste("When pit.type='two-sided' or pi.type='upper' ",
                   "all values of 'n.plus.one.minus.upl.rank' must ",
